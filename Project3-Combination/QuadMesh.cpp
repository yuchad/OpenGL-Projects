...
//Function added to QuadMesh
/*void QuadMesh::UpdateMesh(int meshSize,VECTOR3D mousePos,int b,int a) {
	int currentVertex = 0;
	float r = 0; 
	...
	for (int i = 0; i < meshSize; i++)
	{
		...
		for (int j = 0; j < meshSize; j++)
		{
			...
			r = sqrt(pow(mousePos.x - vertices[currentVertex].position.x, 2) + pow(mousePos.y - vertices[currentVertex].position.y, 2) + pow(mousePos.z - vertices[currentVertex].position.z, 2));
			
			vertices[currentVertex].position.y += b * (pow(e, (-a*pow(r, 2))));
...
void QuadMesh::UpdateMesh(Metaballs * blobList, int blobCount,int meshSize) {
	for (int i = 0; i < meshSize +1 ; i++)
	{
		for (int j = 0; j < meshSize +1 ; j++)
		{
			VECTOR3D pos = vertices[i*(meshSize +1) + j].position;
			vertices[i*(meshSize + 1) + j].position.y = 0; 
			for (int x = 0; x <= blobCount; x++) {
				float r = 1 + static_cast <float> (rand()) / static_cast <float> (RAND_MAX) / 50; 
				float distance = pow(blobList[x].pos.GetX() - pos.GetX(), 2) + pow(blobList[x].pos.GetZ() - pos.GetZ(), 2) + pow(blobList[x].pos.GetZ() - pos.GetZ(), 2); //r^2
				vertices[i*(meshSize + 1) + j].position.y += (blobList[x].height*r)*exp(-((blobList[x].width / r) * distance));
			}
		}
	}
	this->ComputeNormals();
